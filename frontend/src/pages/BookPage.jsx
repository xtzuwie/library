...
const BookPage = () => {
  ...
  const { fetchBook, book, isLoading, deleteBook } = useLibraryStore();
    ...
    <div className="min-h-screen text-[#252422] bg-[#f5f5f5] px-4 md:px-12 pb-10">
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <div className="mt-6 md:mt-0 md:max-w-4xl basis-[65%]">
          ...
          <p className="pl-5">Author: {book?.author}</p>

          <p className="mt-2 font-semibold text-lg md:text-xl">Book #:</p>
          <p className="md:text-lg">{book?.isbn}</p>
        </div>
      </div>
    </div>
  );
